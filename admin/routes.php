...
$stmt->close();
$profile_image = $admin['profile_image'] ?? 'default.png';
$message = '';

if (isset($_POST['add_city'], $_POST['city_name'])) {
    $city = trim($_POST['city_name']);
    if ($city === '') {
        $message = "City name is required.";
    } else {
        $stmt = $conn->prepare("SELECT route_id FROM routes WHERE LOWER(city_name)=LOWER(?)");
        ...
        if ($stmt->num_rows) {
            ...
            $stmt->bind_param("s", $city);
            $message = $stmt->execute() ? "City added successfully." : "Failed to add city.";
        }
        $stmt->close();
    }
}
...
if (isset($_POST['update_city'])) {
    ...
    if ($city === '') {
        ...
        $stmt = $conn->prepare("SELECT route_id FROM routes WHERE LOWER(city_name)=LOWER(?) AND route_id!=?");
        ...
        if ($stmt->num_rows) {
            ...
            $stmt->bind_param("si", $city, $id);
            $message = $stmt->execute() ? "City updated successfully." : "Failed to update city.";
        }
        $stmt->close();
    }
}
...
if (isset($_GET['delete'])) {
    ...
    $message = $stmt->execute() ? "City deleted successfully." : "Failed to delete city.";
    $stmt->close();
}
...
if (isset($_GET['edit'])) {
    ...
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $edit_city = $stmt->get_result()->fetch_assoc();
    $stmt->close();
}

$routes = $conn->query("SELECT route_id,city_name,created_at FROM routes ORDER BY route_id DESC");
?>
<!DOCTYPE html>
<html lang="en">
