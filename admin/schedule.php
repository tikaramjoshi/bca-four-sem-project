...
if (isset($_POST['add_schedule']) || isset($_POST['update_schedule'])) {
    $edit = isset($_POST['update_schedule']);
    ...
    if ($bus_id <= 0) {
        ...
        if ($message === "") {
            if ($stmt->execute()) {
                $message = $edit ? "Schedule updated successfully." : "Schedule added successfully.";
                $type = "success";
            } else {
                $message = $edit ? "Failed to update schedule." : "Failed to add schedule.";
                $type = "error";
            }
            $stmt->close();
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $message = $stmt->execute() ? "Schedule deleted successfully." : "Failed to delete schedule.";
            ...
            <div class="schedule-page">
                ...
                <div class="schedule-form-box">
                    ...
                    <form method="POST" class="schedule-form">
                        <?php if ($edit_schedule): ?><input type="hidden" name="schedule_id" value="<?= ((int)$edit_schedule['schedule_id']) ?>"><?php endif; ?>
                        <div class="form-group">
                            <label>Bus</label>
                            <select name="bus_id" required>
                                <option value="">Select Bus</option>
                                <?php while ($bus = $buses->fetch_assoc()): ?>
                                    <option value="<?= ($bus['bus_id']) ?>" <?= $edit_schedule && $edit_schedule['bus_id'] == $bus['bus_id'] ? 'selected' : '' ?>><?= htmlspecialchars($bus['bus_number'] . ' - ' . $bus['bus_name']) ?> (<?= ($bus['seats']) ?> seats)</option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="from_city" required>
                                ...
                                <?php while ($route = $routes->fetch_assoc()): ?>
                                    <option value="<?= htmlspecialchars($route['city_name']) ?>" <?= ($edit_schedule['from_city'] ?? '') == $route['city_name'] ? 'selected' : '' ?>><?= htmlspecialchars(ucwords($route['city_name'])) ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="to_city" required>
                                ...
                                ?>
                                    <option value="<?= htmlspecialchars($route['city_name']) ?>" <?= ($edit_schedule['to_city'] ?? '') == $route['city_name'] ? 'selected' : '' ?>><?= htmlspecialchars(ucwords($route['city_name'])) ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Departure Date</label>
                            <input type="date" name="departure_date" value="<?= htmlspecialchars($edit_schedule['departure_date'] ?? '') ?>" min="<?= $today ?>" max="<?= $max_date ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Departure Time</label>
                            <input type="time" name="departure_time" value="<?= htmlspecialchars($edit_schedule['departure_time'] ?? '') ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Ticket Price</label>
                            <input type="number" name="ticket_price" value="<?= htmlspecialchars($edit_schedule['ticket_price'] ?? '') ?>" min="0" step="0.01" required>
                        </div>
                        <div class="form-group">
                            <label>Available Seats</label>
                            <input type="number" name="available_seats" value="<?= htmlspecialchars($edit_schedule['available_seats'] ?? '') ?>" min="0" required>
                        </div>
                        <div class="form-group">
                            <label>Status</label>
                            <select name="status">
                        ...
                        <div class="form-actions">
                            <?php if ($edit_schedule): ?>
                                <button type="submit" name="update_schedule" class="btn btn-primary"><i class="fa fa-save"></i> Update Schedule</button>
                                <a href="schedule.php" class="btn btn-cancel">Cancel</a>
                            <?php else: ?>
                                <button type="submit" name="add_schedule" class="btn btn-primary"><i class="fa fa-plus"></i> Add Schedule</button>
                            <?php endif; ?>
                        </div>
                    </form>
                </div>
                                ...
                                <?php if ($schedules && $schedules->num_rows > 0): ?>
                                    <?php while ($row = $schedules->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="schedule.php?edit=<?= ($row['schedule_id']) ?>" class="edit"><i class="fa fa-edit"></i> Edit</a>
                                                <a href="schedule.php?delete=<?= ($row['schedule_id']) ?>" class="delete" onclick="return confirm('Are you sure you want to delete this schedule?')"><i class="fa fa-trash"></i> Delete</a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                <?php else: ?>
