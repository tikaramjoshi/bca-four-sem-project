...
$stmt = $conn->prepare("SELECT b.bus_id,b.bus_number,b.bus_name,b.bus_type,b.seats,b.status FROM bus_driver bd INNER JOIN bus b ON bd.bus_id=b.bus_id WHERE bd.driver_id=? AND b.status='approved' LIMIT 1");
$stmt->bind_param("i", $driver_id);
$stmt->execute();
...
$today_schedule = [];
if ($bus) {
    $stmt = $conn->prepare("SELECT schedule_id,bus_id,from_city,to_city,departure_date,departure_time,ticket_price,available_seats,status FROM schedule WHERE bus_id=? AND departure_date=? AND status!='cancelled' ORDER BY departure_time ASC");
...
$notifications = [];
if ($bus) $notifications[] = ["title" => "Bus Assigned", "message" => "You are assigned to bus " . $bus['bus_number'], "time" => "Today"];
$notifications[] = ["title" => !empty($today_schedule) ? "Today's Trip" : "No Trip", "message" => !empty($today_schedule) ? "You have " . count($today_schedule) . " scheduled trip(s) today." : "You have no scheduled trip for today.", "time" => "Today"];
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container">
        ...
        <div class="cards">
            <div class="card">
                ...
                <div class="card-value"><?= $bus ? htmlspecialchars($bus['bus_number']) : 'None' ?></div>
            </div>
            <div class="card">
        ...
        <div class="content-grid">
            <div>
                <div class="box">
                    ...
                    <?php if ($bus): ?>
                        <div class="bus-details">
                            <div class="bus-item"><small>Bus Number</small><strong><?= htmlspecialchars($bus['bus_number']) ?></strong></div>
                            <div class="bus-item"><small>Bus Name</small><strong><?= htmlspecialchars($bus['bus_name']) ?></strong></div>
                            <div class="bus-item"><small>Bus Type</small><strong><?= htmlspecialchars($bus['bus_type']) ?></strong></div>
                            <div class="bus-item"><small>Total Seats</small><strong><?= htmlspecialchars($bus['seats']) ?></strong></div>
                            <div class="bus-item"><small>Bus Status</small><strong><?= ucfirst(htmlspecialchars($bus['status'])) ?></strong></div>
                        </div>
                    <?php else: ?><div class="not-assigned">No bus has been assigned to you yet.</div><?php endif; ?>
                </div>
                <div class="box">
                    <h2>Today's Trips</h2>
                ...
                <div class="box">
                    ...
                    <div class="bus-details">
                        <div class="bus-item"><small>Name</small><strong><?= htmlspecialchars($driver['name']) ?></strong></div>
                        <div class="bus-item"><small>Phone</small><strong><?= htmlspecialchars($driver['phone']) ?></strong></div>
                        <div class="bus-item"><small>Email</small><strong><?= htmlspecialchars($driver['email']) ?></strong></div>
                        <div class="bus-item"><small>Verification</small><?php if (isset($driver['verification_status']) && $driver['verification_status'] === 'verified'): ?><span class="verified">Verified ✓</span><?php else: ?><span class="unverified">Unverified</span><?php endif; ?></div>
                    </div>
                </div>
            </div>
            <div>
                <div class="box">
                    <h2>Driver Status</h2>
                    <div class="status-row"><span class="status-dot"></span><span class="status-text"><?= htmlspecialchars($driver_status) ?></span></div>
                    <p style="margin-top:15px;color:#777;font-size:14px;line-height:1.6;">Your current driver status is based on today's scheduled departure time.</p>
                </div>
                <div class="box">
                    <h2>Verification Status</h2>
                    <div class="verification-box">
                        <?php if (isset($driver['verification_status']) && $driver['verification_status'] === 'verified'): ?>
                ...
                <div class="box">
                    ...
                    <?php if (!empty($notifications)): foreach ($notifications as $notification): ?>
                            <div class="notification"><strong><?= htmlspecialchars($notification['title']) ?></strong>
                                <p><?= htmlspecialchars($notification['message']) ?></p><small><?= htmlspecialchars($notification['time']) ?></small>
                            </div>
                        <?php endforeach;
                    else: ?><div class="empty">No notifications.</div><?php endif; ?>
                </div>
                <div class="box">
                    <h2>Passenger Summary</h2>
                    <div class="bus-details">
                        <div class="bus-item"><small>Today's Passengers</small><strong><?= $today_bookings ?></strong></div>
                        <div class="bus-item"><small>Total Bookings</small><strong><?= $total_bookings ?></strong></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener("click", function(e) {
            const p = document.querySelector(".driver-profile");
            if (p && !p.contains(e.target)) p.classList.remove("active");
        });
    </script>
</body>
